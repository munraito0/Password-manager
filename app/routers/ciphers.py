...
@router.get("/", response_model=list[CipherResponse])
async def get_ciphers(db: AsyncSession = Depends(get_db)):
    return await cipher_service.get_all(db)

@router.get("/{cipher_id}", response_model=CipherResponse)
...
@router.delete("/{cipher_id}", status_code=204)
