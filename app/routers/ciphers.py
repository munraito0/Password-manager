...
async def get_cipher(cipher_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    if not cipher:
        logger.warning(
            f"Cipher {cipher_id} not found",
            extra={"method": "GET", "path": f"/api/ciphers/{cipher_id}", "statusCode": 404},
        )
        raise NotFoundException(f"Cipher {cipher_id} not found")
    return cipher
...
async def create_cipher(data: CipherCreate, db: AsyncSession = Depends(get_db)):
    cipher = await cipher_service.create(db, data)
    logger.info(
        "Cipher successfully created",
        extra={"method": "POST", "path": "/api/ciphers", "statusCode": 201},
    )
    return cipher
...
async def update_cipher(cipher_id: uuid.UUID, data: CipherUpdate, db: AsyncSession = Depends(get_db)):
    ...
    if not cipher:
        raise NotFoundException(f"Cipher {cipher_id} not found")
    logger.info(
        f"Cipher {cipher_id} successfully updated",
        extra={"method": "PUT", "path": f"/api/ciphers/{cipher_id}", "statusCode": 200},
    )
    return cipher
...
async def delete_cipher(cipher_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    ...
    if not deleted:
        raise NotFoundException(f"Cipher {cipher_id} not found")
    logger.info(
        f"Cipher {cipher_id} successfully deleted",
        extra={"method": "DELETE", "path": f"/api/ciphers/{cipher_id}", "statusCode": 204},
    )
